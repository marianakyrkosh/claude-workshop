...
- (Optional) Figma node IDs captured in the change request — if the Figma MCP is configured, call `get_design_context` to pull layouts, component references, and design tokens into the design doc. Skip if no Figma context is available.
